 ...
 *
 * Derived from A-Frame Extras keyboard-controls.js.
 * https://github.com/n5ro/aframe-extras/blob/master/src/controls/keyboard-controls.js
 *
 * Define a set of events, and bind each event to one or more keys.
 * When a bound key is pressed, the event will be emitted.
 *
 * This is intended for use with any object that you wish to control via a keyboard.
 * Useful for debugging aas well as for use in production.
 *
 * Following text lifted direct from keyboard-control.js, explaining some of the
 * complexities of keyboards that this component helps to abstract away.
 *
 ...
 */
AFRAME.registerComponent('key-bindings', {
  schema: {
    debug:             {default: false },
    bindings:          {default: ["KeyW=forward", "KeyA=left", "KeyD=right", "KeyS=back"]}
  },

  init: function () {

    /* localKeys is used to store a record of which keys are currently pressed down
     * This was inherited from keyboard-control.js.
     * The function is not currently used, but it seemed worth keeping as potentially
     * useful in future...
     */
    this.localKeys = {};
    this.listeners = {
    ...
    /* This is where we store the key-event bindings that we've been programmed with */
    this.boundKeys = [];
    this.boundEvents = [];
  },
  ...
  * Events
  *
  * According to A-Frame 1.1.0 docs, these could be declared using events
  * objects.
  * Leaving as is for now to minimize change while I get this working.
  * Maybe will tidy this later.
  */
  ...
  /*******************************************************************
  * Accessors.
  * Proxy controls is derived from A-Frame Extras keyboard-controls.js
  *
  * From the README.md:
  * (https://github.com/n5ro/aframe-extras/blob/master/src/controls/README.md)
  * Mobile + Desktop Input Devices
  * Connect input devices from your desktop to your mobile phone with
  * WebRTC, using ProxyControls.js.
  *
  * I have not tested this, so no idea if it works, but leaving the code in,
  * since it might be useful for someone.
  */
...
/**
 * The remaining content is taken directly from
 * https://github.com/donmccurdy/aframe-keyboard-controls/blob/master/lib/keyboard.polyfill.js
 * which in turn is (I believe) a direct copy of:
